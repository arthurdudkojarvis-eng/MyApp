...
final class AddHoldingTests: XCTestCase {
    ...
    private func simulateSave(
        ...
        let holding = Holding(shares: shares, averageCostBasis: costBasis, purchaseDate: purchaseDate)
    ...
    func testSaveInsertsOneHolding() throws {
        try simulateSave(ticker: "AAPL", shares: 10, costBasis: 150)
    ...
    func testSaveStoresSharesAndCostBasis() throws {
        try simulateSave(ticker: "VTI", shares: Decimal(string: "42.5")!, costBasis: Decimal(string: "220.75")!)
        let holdings = try context.fetch(FetchDescriptor<Holding>())
    ...
    func testSaveStoresPurchaseDate() throws {
        ...
        try simulateSave(ticker: "O", shares: 50, costBasis: 55, purchaseDate: date)
        let holdings = try context.fetch(FetchDescriptor<Holding>())
        XCTAssertEqual(
    ...
    func testTickerIsUppercasedOnSave() throws {
        try simulateSave(ticker: "aapl", shares: 1, costBasis: 100)
    ...
    func testTickerIsTrimmedOnSave() throws {
        try simulateSave(ticker: "  MSFT  ", shares: 1, costBasis: 400)
        let stocks = try context.fetch(FetchDescriptor<Stock>())
    ...
    func testSavingTwoHoldingsSameTickerSharesOneStock() throws {
        ...
        try simulateSave(ticker: "VYM", shares: 10, costBasis: 100)
    ...
    private func isValid(ticker: String, sharesText: String, costBasisText: String) -> Bool {
    ...
    func testIsValidWithAllFieldsPopulated() {
        XCTAssertTrue(isValid(ticker: "AAPL", sharesText: "10", costBasisText: "150"))
    }

    func testIsValidFalseWhenTickerEmpty() {
        XCTAssertFalse(isValid(ticker: "", sharesText: "10", costBasisText: "150"))
    }

    func testIsValidFalseWhenSharesZero() {
        XCTAssertFalse(isValid(ticker: "AAPL", sharesText: "0", costBasisText: "150"))
    }

    func testIsValidFalseWhenSharesNegative() {
        XCTAssertFalse(isValid(ticker: "AAPL", sharesText: "-5", costBasisText: "150"))
    }

    func testIsValidFalseWhenCostBasisZero() {
        XCTAssertFalse(isValid(ticker: "AAPL", sharesText: "10", costBasisText: "0"))
    }

    func testIsValidFalseWhenSharesNotNumeric() {
        XCTAssertFalse(isValid(ticker: "AAPL", sharesText: "abc", costBasisText: "150"))
    }
    ...
    func testIsValidFalseWhenCostBasisNotNumeric() {
        XCTAssertFalse(isValid(ticker: "AAPL", sharesText: "10", costBasisText: "abc"))
    }
